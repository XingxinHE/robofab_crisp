#!/usr/bin/env python3
"""Preflight checks for FR3 ACT deployment with 3 cameras."""
...
PRIMARY_ENV_CONFIG = "fr3_3cams_deploy_streamed"
FALLBACK_ENV_CONFIG = "fr3_3cams_deploy_streamed_joint_states_fallback"
...
def _select_env_config(topic_set: set[str], namespace: str) -> tuple[str, str]:
    ...
    return (
        FALLBACK_ENV_CONFIG,
        f"falling back to {joint_topic} with gripper index 7",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"[deploy-preflight-3cams] FAIL: {exc}", file=sys.stderr)
        return 1

    missing, silent = _check_topic_messages(topic_set, namespace=args.namespace)
    if missing:
        print(
            "[deploy-preflight-3cams] FAIL: missing required topics:", file=sys.stderr
        )
    ...
    if silent:
        print(
            "[deploy-preflight-3cams] FAIL: required topics are visible but not publishing data:",
            file=sys.stderr,
        )
        for topic in silent:
            print(f"  - {topic}", file=sys.stderr)
    ...
    print("[deploy-preflight-3cams] PASS")
    print(f"[deploy-preflight-3cams] namespace: {args.namespace or '<root>'}")
    print(f"[deploy-preflight-3cams] env config: {env_config} ({reason})")
    return 0
