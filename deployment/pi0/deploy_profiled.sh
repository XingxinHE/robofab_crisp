...
if [[ -z "${PROFILE_NAME}" || -z "${DEFAULT_REPO_ID}" || -z "${PREFLIGHT_MODULE}" ]]; then
  echo "[deploy-pi0-profiled] Missing required profile parameters." >&2
  exit 2
fi
...
if [[ "${1:-}" == "--help" ]]; then
  cat <<EOF
Deploy Pi0 with profile '${PROFILE_NAME}'.
...
if [[ " $* " == *" --policy-config "* ]]; then
  echo "[${PROFILE_NAME}] Do not pass --policy-config. Pi0 deployment is pinned by this wrapper." >&2
  exit 1
fi
...
for path in Path("outputs/train").glob("**/pretrained_model"):
    try:
        if json.loads((path / "config.json").read_text()).get("type") == "pi0":
            models.append(path)
    except Exception:
        pass
...
if [[ -z "${MODEL_PATH}" ]]; then
  echo "[${PROFILE_NAME}] Could not find a Pi0 model automatically. Pass --model-path explicitly." >&2
  exit 1
fi
...
if policy_type != "pi0":
    raise SystemExit(f"Expected a Pi0 checkpoint, got policy type {policy_type!r}: {model_path}")
PY
...
echo "[${PROFILE_NAME}] Using Pi0 model: ${MODEL_PATH}"
...
exec python -m deployment.pi0.deploy_policy \
  --repo-id "${REPO_ID}" \
