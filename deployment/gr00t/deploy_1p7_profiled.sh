...
if [[ -z "${PROFILE_NAME}" || -z "${DEFAULT_REPO_ID}" || -z "${PREFLIGHT_MODULE}" ]]; then
  echo "[deploy-gr00t-1p7-profiled] Missing required profile parameters." >&2
  exit 2
fi
...
if [[ "${1:-}" == "--help" ]]; then
  cat <<EOF
Deploy GR00T N1.7 (DROID) with profile '${PROFILE_NAME}'.

Start the GR00T N1.7 server separately, then run this client.

Usage:
  ZMQ (N1.7): pixi run ${PROFILE_NAME} -- --groot-transport zmq1p7 --groot-host 127.0.0.1 --groot-port 5555 [extra args]
...
Defaults:
  ...
  --gripper-flip                  Optional open/close inversion for both modes
  --gripper-hysteresis <x>        Optional threshold hysteresis
  --gripper-debounce-steps <n>    Optional consecutive-frame gripper debounce
  --gripper-uncertain-band <x>    Optional band around gripper threshold
  --gripper-uncertain-policy <p>  none|hold|open|closed
...
Joint-space mode:
  The 'deploy-gr00t-1p7-fr3-3cams-gamepad-joint' pixi task already passes
  --joint-control --fps 15 --action-chunk-size 15 --async-inference
  --prefetch-threshold 5. Do NOT use a '--' separator for that task; just
  append extra args directly. Other joint knobs: --max-joint-step-rad,
  --joint-target-smoothing-alpha, --joint-delta-smoothing-alpha.
EOF
  exit 0
fi
...
if [[ " $* " == *" --policy-config "* ]]; then
  echo "[${PROFILE_NAME}] Do not pass --policy-config. GR00T N1.7 deployment is pinned by this wrapper." >&2
  exit 1
fi
...
if [[ "${GROOT_TRANSPORT}" == "zmq" ]]; then
  echo "[${PROFILE_NAME}] Using GR00T N1.7 ZMQ server: tcp://${GROOT_HOST}:${GROOT_PORT}"
else
  echo "[${PROFILE_NAME}] Using GR00T N1.7 HTTP server: ${GROOT_SERVER}"
fi
...
exec python -m deployment.gr00t.deploy_policy_1p7 \
  --repo-id "${REPO_ID}" \
