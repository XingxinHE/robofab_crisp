"""Deploy a remote GR00T N1.7 (DROID) policy with robofab-specific homing controls."""
...
from deployment.gr00t.gr00t_1p7_remote_joint_policy import Gr00t1p7RemoteJointPolicy
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Deploy a GR00T N1.7 (DROID) policy through a remote inference server"
    )
    ...
    parser.add_argument(
        "--joint-control",
        action="store_true",
        help="Use CRISP joint-space control instead of Cartesian control.",
    )
    parser.add_argument(
        "--max-joint-step-rad",
        type=float,
        default=0.2,
        help="Maximum joint delta (rad) to send per control step in joint-control mode.",
    )
    parser.add_argument(
        "--joint-target-smoothing-alpha",
        type=float,
        default=1.0,
        help=(
            "Joint mode only. Low-pass smoothing alpha for GR00T joint targets; "
            "1.0 disables smoothing."
        ),
    )
    parser.add_argument(
        "--joint-delta-smoothing-alpha",
        type=float,
        default=1.0,
        help=(
            "Joint mode only. Low-pass smoothing alpha for executed joint deltas; "
            "1.0 disables smoothing."
        ),
    )
    parser.add_argument(
        "--gripper-flip",
        action="store_true",
        help="Invert the GR00T gripper open/close mapping before command postprocessing.",
    )
    parser.add_argument(
        "--gripper-threshold",
        type=float,
        default=0.5,
        help="Threshold used for GR00T gripper open/close decisions.",
    )
    parser.add_argument(
        "--gripper-hysteresis",
        type=float,
        default=0.0,
        help=(
            "Optional gripper hysteresis around --gripper-threshold. "
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Require this many consecutive opposite gripper decisions before switching. "
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Optional band around --gripper-threshold treated as uncertain. "
            "0.0 preserves legacy behavior."
        ),
    )
    parser.add_argument(
        ...
        default="none",
        help=(
            "What to do inside --gripper-uncertain-band. "
            "'none' preserves legacy behavior; 'hold' keeps the previous decision."
        ),
    )
    parser.add_argument(
        "--log-level",
    ...
    parser.add_argument(
        "--groot-transport",
        choices=["http", "zmq", "zmq1p7"],
        default="zmq1p7",
        help=(
            "Transport used to talk to the GR00T N1.7 inference server. "
            "Use 'zmq1p7' for the N1.7 PolicyServer (msgpack_numpy); "
            "'zmq' is the legacy N1.5 torch-serialized protocol."
        ),
    )
    parser.add_argument(
        "--groot-server",
        ...
        help="HTTP URL of the GR00T N1.7 inference server.",
    )
    parser.add_argument(
        "--groot-host",
        type=str,
        default="127.0.0.1",
        help="ZMQ host for the GR00T N1.7 inference server.",
    )
    parser.add_argument(
        "--groot-port",
        type=int,
        default=5555,
        help="ZMQ port for the GR00T N1.7 inference server.",
    )
    parser.add_argument(
        "--groot-api-token",
        type=str,
        default=None,
        help="Optional API token for GR00T N1.7 ZMQ requests.",
    )
    parser.add_argument(
        "--task",
        type=str,
        default=DEFAULT_GROOT_1P7_TASK,
        help="Language task prompt passed to GR00T N1.7.",
    )
    parser.add_argument(
        "--action-chunk-size",
        type=int,
        default=1,
        help="Number of GR00T N1.7 horizon actions to execute before requesting a new horizon.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.08,
        help="(Cartesian mode only) Max gripper width used to normalize gripper commands.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Prefetch GR00T N1.7 horizons in a background thread while queued actions execute.",
    )
    parser.add_argument(
        "--prefetch-threshold",
        type=int,
        default=None,
        help=(
            "Start a background GR00T N1.7 request when queued actions are at or below this count. "
    ...
    parser.add_argument(
        ...
        default=True,
        help="Check GR00T N1.7 /health before deployment starts.",
    )
    parser.add_argument(
        "--home-config",
        type=str,
...
def evaluation_output_file(args: argparse.Namespace) -> str:
    ...
    return (
        prompt.prompt(
            ...
            default=f"evaluation_results_gr00t_1p7_{safe_server}_{datetime_now}",
        )
        + ".csv"
    )
...
def main() -> int:
    ...
    try:
        ...
        control_type = "joint" if args.joint_control else "cartesian"
        env = make_env(
            args.env_config,
        ...
        logger.info("Setting up the GR00T N1.7 remote policy (%s control).", control_type)
        if args.joint_control:
            policy = Gr00t1p7RemoteJointPolicy(
                env=env,
                ...
                joint_target_smoothing_alpha=args.joint_target_smoothing_alpha,
                joint_delta_smoothing_alpha=args.joint_delta_smoothing_alpha,
                ...
                gripper_uncertain_band=args.gripper_uncertain_band,
                gripper_uncertain_policy=args.gripper_uncertain_policy,
        ...
        logger.info("Homing robot before starting GR00T N1.7 deployment recording.")
        ...
        logger.info("Shutting down GR00T N1.7 client.")
        policy.shutdown()
        policy = None

        logger.info("Homing robot after GR00T N1.7 deployment.")
        ...
        env.close()
        env = None
        logger.info("Finished GR00T N1.7 deployment recording.")
        return 0
