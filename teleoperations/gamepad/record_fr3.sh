...
Usage:
  pixi run record-gamepad-fr3 -- --repo-id <repo_id> [extra args]
...
if [[ " $* " == *" --follower-config "* ]]; then
  echo "[record-gamepad-fr3] Do not pass --follower-config. It is selected automatically by preflight." >&2
  exit 1
fi
...
if [[ -z "${FOLLOWER_CONFIG}" ]]; then
  echo "[record-gamepad-fr3] Failed to determine follower config from preflight." >&2
  exit 1
fi

echo "[record-gamepad-fr3] Using follower config: ${FOLLOWER_CONFIG}"
